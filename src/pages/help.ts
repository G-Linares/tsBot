import {APIEmbedField, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, InteractionReplyOptions, SelectMenuBuilder, SelectMenuOptionBuilder} from "discord.js"
import CategoryRoot from "../commands"
import { createId, readId } from './../utils/interaction';
...
// Omit this, is only for dev purposes
const N = Namespaces
const A = Actions
...
export function getCategoryPage( interactionId: string): InteractionReplyOptions {
    // Exctract needed metadata from interactionId
    const [_namespace, categoryName, action, currentOffset] = readId(interactionId)
    // An expected action coudl look like this
    // Namespaces;Debug;+;2
    ...
    if(!category)
        throw new Error('Invalid intersectionId; Failed to find corresponding category page!')

    // Get current offset
    let offset = parseInt(currentOffset);
    // if is NaN set offset to 0
    if(isNaN(offset)) offset = 0;
    // Increment or decrement offset accoprding to action
    // We use the else if because action could be undefined
    if( action === A.next){
        offset++;
